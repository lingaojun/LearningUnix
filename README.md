...
msgsnd() 消息队列放置函数，可理解为用于存放队列信息。第一个参数是消息队列ID,由msgget()返回。第二个参数是具体消息(void *)。第三个参数是消息长度。第四个参数是表示发送消息的配置参数，例如IPC_NOWAIT(无阻塞)。**一般来说消息由三部分组成：一个正的长整型的字段，一个非负的长度(第三个参数)，以及实际数据。所以要想构成三部分，第二个入参中需要传入一个特定的结构体，该结构体需要由一个长整型字段(int)和一个数据(char[])组成。**返回值若成功为0，失败-1。  
